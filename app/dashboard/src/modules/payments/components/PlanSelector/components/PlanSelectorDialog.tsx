...
import { PRICE_CURRENCY, PRICE_PER_PLAN } from '../../../constants'
...
function Summary(props: SummaryProps) {
  const { plan, seats, price, formatter, isInvalid = false } = props
  const { getText } = useText()

  const { data, isLoading, isError, refetch, error } = useQuery({
    queryKey: ['getPrice', { plan, seats }],
    enabled: !isInvalid,
    queryFn: () =>
      Promise.resolve({ monthly: price * seats, billingPeriod: 12, total: price * seats * 12 }),
  })
